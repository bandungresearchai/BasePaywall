...
import { useUserUnlockedContent, usePaywallPrice } from '@/hooks/usePaywall';
    ...
    <div className="card flex items-center justify-between p-4">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Paid {priceInEth.toFixed(4)} ETH</p>
        </div>
      </div>
...
function EmptyState() {
  return (
    <div className="text-center py-8">
      ...
      <p className="text-gray-500 text-sm mt-2">Pay to unlock premium content and it will appear here.</p>
    </div>
  );
}
...
export function UserUnlockedContent() {
  const { isConnected } = useAccount();
  const { unlockedContent, unlockCount, isLoading } = useUserUnlockedContent();
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {unlockCount > 0 && (
          <span className="bg-base-blue/20 text-base-blue px-3 py-1 rounded-full text-sm">
            {unlockCount} item{unlockCount !== 1 ? 's' : ''}
          </span>
        )}
      </div>
